...
public class CsjRenderFeedAdapter extends AdvanceSelfRenderCustomAdapter {
    TTVfNative mTTVfNative;
    TTVfObject mRenderAD;
    ...
    private void startLoad() {
        try {
            //检查是否命中使用缓存逻辑
            boolean hitCache = AdvanceCacheUtil.loadWithCacheData(this, TTVfObject.class, new BYAbsCallBack<TTVfObject>() {
                @Override
                public void invoke(TTVfObject cacheAD) {
                    mRenderAD = cacheAD;
            ...
            mTTVfNative = ttAdManager.createVfNative(getRealContext());
            ...
            //请求自渲染广告
            mTTVfNative.loadVfList(adSlot, new TTVfNative.VfListListener() {
                @Override
...
//                        mAdvanceRFBridge.adapterDidLoaded(dataConverter);
    ...
    private void startShow() {
        try {
            if (mAdvanceRFBridge == null || mTTVfNative == null || mRenderAD == null) {
            ...
            if (dislikeView != null) {
                final TTVfDislike ttAdDislike = mRenderAD.getDislikeDialog(getRealActivity(rfMaterialProvider.rootView));
                ...
                ttAdDislike.setDislikeInteractionCallback(new TTVfDislike.DislikeInteractionCallback() {
                    @Override
                    public void onShow() {
...
//            绑定view
            mRenderAD.registerViewForInteraction((ViewGroup) rfMaterialProvider.rootView, rfMaterialProvider.clickViews, rfMaterialProvider.creativeViews, rfMaterialProvider.disLikeView, new TTNtObject.AdInteractionListener() {
                @Override
            ...
            if (dataConverter != null && dataConverter.isDownloadAD() && downloadListener != null) {
            ...
            if (dataConverter != null && dataConverter.isVideo()) {
                ...
                mRenderAD.setVideoListener(new TTVfObject.VideoVfListener() {
                    @Override
